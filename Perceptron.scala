  ...
  def retroPropag(observe_ : Array[Double], souhaite_ : Array[Double], pas_ : Double = 0.1): Unit = {
    ...
    for(i <- inputI.length-2 until 0 by -1) {
      for(j <- 0 until inputI(i).length) {
        ...
        for(k <- 0 until poids(i-1)(j).length-1) { //Calcul des autres poids
          poids(i-1)(j)(k) = poids(i-1)(j)(k) - pas_ * dI(i)(j) * outputI(i-1)(k)
        }
      }
    }
...
object Main {
  def main(args: $[String]) = {
    if(args.length <= 0){
      println("mauvaise utilisation : ")
      println("-h pour avoir de l'aide : scala Perceptron.jar -h ")

    }else{
      val monPerceptron = Perceptron( 2,4,1)
      args(0) match {
        case "-h" => {
          println("usage : scala Perceptron.jar <list> ")
          println("le perceptron est (2,4,1)")
          println("Remplacer <list> par xOR , OR ou AND")
          println("Pour afficher le taux d'erreur jusqu'à obtenir un taux d'erreur < 0.01 ")
        }
        case "OR" => {
        ...
        case "xOR" => {
          ...
          var i = 0;
          while( i <= 10000) {
            print(i + " " +monPerceptron.erreur(xOR) + "\n");
